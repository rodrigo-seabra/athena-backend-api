...
class StudentStatsController {
  public async getSemesterPerformance(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (!studentStats) {
        return res.status(404).json({ message: "Current student stats not found." });
      }
  
      // Pega o último semestre disponível em `pastStat`
      const pastStats = studentStats.pastStat?.[studentStats.pastStat.length - 1];
  
      const performanceData = studentStats.subjects.map(subject => {
        const currentPercentage = (subject.averageLevel / 5) * 100;
        const pastAverageLevel = pastStats?.subjects.find(s => s.name === subject.name)?.averageLevel || 0;
        const pastPercentage = (pastAverageLevel / 5) * 100;
